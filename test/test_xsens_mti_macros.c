...
void test_optionflag_set( void )
{
    uint32_t options = 0;

    // Set DisableAutoStore
    TEST_ASSERT_EQUAL( 0x00000001, XSENS_OPTION_FLAG_SET(options, XSENS_OPTFLAG_DISABLE_AUTOSTORE) );
    options = 0;

    // Set DisableAutoMeasurement
    TEST_ASSERT_EQUAL( 0x00000002, XSENS_OPTION_FLAG_SET(options, XSENS_OPTFLAG_DISABLE_AUTOMEASUREMENT) );
    options = 0;

    // Set AHS
    TEST_ASSERT_EQUAL( 0x00000010, XSENS_OPTION_FLAG_SET(options, XSENS_OPTFLAG_ENABLE_AHS) );
    options = 0;

    // Set all three of the above at once
    XSENS_OPTION_FLAG_SET(options, XSENS_OPTFLAG_DISABLE_AUTOSTORE);
    XSENS_OPTION_FLAG_SET(options, XSENS_OPTFLAG_DISABLE_AUTOMEASUREMENT);
    XSENS_OPTION_FLAG_SET(options, XSENS_OPTFLAG_ENABLE_AHS);
    TEST_ASSERT_EQUAL( 0x00000013, options );
}

void test_optionflag_is_set( void )
{
    uint32_t options = 0;

    TEST_ASSERT_FALSE( XSENS_OPTION_FLAG_IS_ANY_SET(options) );

    // Set DisableAutoStore
    options = 0x00000001;
    TEST_ASSERT_TRUE( XSENS_OPTION_FLAG_IS_SET(options, XSENS_OPTFLAG_DISABLE_AUTOSTORE) );
    TEST_ASSERT_TRUE( XSENS_OPTION_FLAG_IS_ANY_SET(options) );

    // Set DisableAutoMeasurement
    options = 0x00000002;
    TEST_ASSERT_TRUE( XSENS_OPTION_FLAG_IS_SET(options, XSENS_OPTFLAG_DISABLE_AUTOMEASUREMENT) );
    TEST_ASSERT_TRUE( XSENS_OPTION_FLAG_IS_ANY_SET(options) );
    TEST_ASSERT_FALSE( XSENS_OPTION_FLAG_IS_SET(options, XSENS_OPTFLAG_DISABLE_AUTOSTORE) );

    // Set AHS
    options = 0x00000010;
    TEST_ASSERT_TRUE( XSENS_OPTION_FLAG_IS_SET(options, XSENS_OPTFLAG_ENABLE_AHS) );
    TEST_ASSERT_TRUE( XSENS_OPTION_FLAG_IS_ANY_SET(options) );
    TEST_ASSERT_FALSE( XSENS_OPTION_FLAG_IS_SET(options, XSENS_OPTFLAG_DISABLE_AUTOMEASUREMENT) );
}